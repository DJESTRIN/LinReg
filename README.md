<h1><b>infeR: An automatic statistics pipeline (Python + R)</b></h1>

infeR turns a raw dataset into a full statistical analysis with a single CLI
...
3. **Run EDA** â€” normality testing (Shapiro-Wilk / Anderson-Darling),
...
4. **Decide a model family** â€” `lm`, `glm` (gaussian/poisson/binomial/gamma/
   ...
   MANOVA/multivariate mixed models for multiple dependent variables â€” using
...
6. **Render a report** (`report.html` + `report.md`) with the formula used,
   ANOVA/coefficient tables, RÂ², AIC/BIC, post-hoc results, effect sizes, and
   embedded diagnostic plots.

Python is the orchestrator (CLI, data wrangling, decision logic, reporting).
R is the statistics engine (model fitting, diagnostics, plots) â€” the two
...
<h2><b>Quick start (Docker â€” recommended)</b></h2>

```powershell
docker compose -f docker/docker-compose.yml build
docker compose -f docker/docker-compose.yml run infer run `
...
```powershell
docker build -f docker/Dockerfile -t infer:latest .
docker run --rm -v ${PWD}/examples:/data -v ${PWD}/out:/results infer:latest `
...
infer run --input ..\examples\mtcars.csv --dv mpg --iv hp,wt --output-dir ..\out\mtcars
```
...
invoking R at all â€” handy for sanity-checking a new dataset first.

<h2><b>Example: repeated-measures mixed model</b></h2>

```powershell
infer run --input examples\repeated_measures_synthetic.csv `
...
```powershell
infer run --input examples\counts_synthetic.csv `
...
* **Tweedie auto-detection**: `infer run --input examples\zero_inflated_tweedie_synthetic.csv --dv y --iv group`
  routes zero-inflated, non-negative continuous outcomes to
  `glmmTMB::glmmTMB(family = tweedie())`.
* **Nonparametric fallback**: `infer run --input examples\nonparametric_synthetic.csv --dv y --iv group`
...
<h2><b>Using infeR from another repository</b></h2>

Since infeR is invoked purely as a CLI (or a container), you can call it
from any other project without importing it as a library â€” either
`pip install -e path\to\infeR\python` and call `infer run ...`, or invoke
...
```
infeR/
  python/        Python orchestrator package (infer_auto) + tests
